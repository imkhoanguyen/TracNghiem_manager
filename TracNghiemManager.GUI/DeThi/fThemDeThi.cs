		...
		private void button1_Click(object sender, EventArgs e)
		{
			if (hanhDong.Equals("add"))
			{
				try
				{
					if (checkValidInput())
					{
						...
						DeThiDTO dt = new DeThiDTO(dtBus.GetAutoIncrement(), txtTenlop.Text, selectedMonHoc.MaMonHoc, 1, time, 1);
			...
			else if (hanhDong.Equals("edit"))
			{
				if (checkValidInput())
				{
					...
					{
						int time = (int)nud.Value;
						DeThiDTO dt = new DeThiDTO(objUpdate.MaDeThi, txtTenlop.Text, selectedMonHoc.MaMonHoc, 1, time, 1);
